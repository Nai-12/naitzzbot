...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const isOwner = ownerNumber == sender ? true : ["6289667440607@s.whatsapp.net"].includes(sender) ? true : false
		...
		const fakereply = ["0@s.whatsapp.net"]
		var cr = `*HAI KAK ${pushname} 👋🏻*`
		...
		switch(command) {
			// Main Menu
			case prefix+'menu':
					var text = `Hallo ${pushname} 👋🏻, Welcome To Bot`
					var but = [{buttonId: `#help`, buttonText: { displayText: "MENU" }, type: 1 }]
					conn.sendMessage(from, { caption: text, image: { url: "./media/naitzz.jpg" }, buttons: but, footer: 'Welcome' }, { quoted: msg })
					break
			case prefix+'help':
			    var teks = allmenu(sender, prefix, pushname, isOwner)
			    var but = [{buttonId: `#speed`, buttonText: { displayText: "SPEED" }, type: 1 }]
				conn.sendMessage(from, { caption: teks, image: { url: "./media/naitzz.jpg" }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: msg })
				break
			case prefix+'runtime':
